...
async function fetchAPI() {
    const baseURL = `https://api.edamam.com/search?q=${searchQery}&app_id=${APP_ID}&app_key=${APP_KEY}&to=8`;
...
function generateHTML(results) {
    ...
    results.map(result => {
        generatedHTML +=
        `<div class="max-w-[300px] bg-gray-800 rounded-md p-5 flex items-center justify-center flex-col">
            <div>
                <img class="rounded-md" src="${result.recipe.image}" alt="">
            </div>
            <div class="pt-5 w-full">
                <div class="flex justify-between">
                    <h3 class="text-2xl whitespace-nowrap overflow-hidden basis-2/5 grow-0">${result.recipe.label}</h3>
